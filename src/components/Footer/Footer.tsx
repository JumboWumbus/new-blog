import Link from "next/link";

import s from "./Footer.module.scss";

interface FooterProps {
  fontClass: string;
}

const Footer: React.FC<FooterProps> = ({ fontClass }) => {
  return (
    <footer className={`${s.footer} ${fontClass}`}>
      <div className={s.container}>
        <div className={s.rightContent}>
          <div className={s.iconContainer}>
            <a
              href="https://twitter.com/GrembloProjects"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>Twitter</span>
              <svg
                className={`${s.icon} ${s.noMargin}`}
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true">
                <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"></path>
              </svg>
            </a>
            <a
              href="https://github.com/JumboWumbus"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>GitHub</span>
              <svg
                className={s.icon}
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true">
                <path
                  fillRule="evenodd"
                  d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
                  clipRule="evenodd"></path>
              </svg>
            </a>
            <a
              hidden={true}
              href="blog"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>HashNode</span>
              <svg
                className={s.icon}
                width="337"
                height="337"
                viewBox="0 0 337 337"
                fill="none">
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M23.155 112.598c-30.873 30.874-30.873 80.93 0 111.804l89.443 89.443c30.874 30.873 80.93 30.873 111.804 0l89.443-89.443c30.873-30.874 30.873-80.93 0-111.804l-89.443-89.443c-30.874-30.873-80.93-30.873-111.804 0l-89.443 89.443zm184.476 95.033c21.612-21.611 21.612-56.651 0-78.262-21.611-21.612-56.651-21.612-78.262 0-21.612 21.611-21.612 56.651 0 78.262 21.611 21.612 56.651 21.612 78.262 0z"
                  fill="currentColor"></path>
              </svg>
            </a>
            <a
              hidden={true}
              href="devto"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>Dev</span>
              <svg
                className={s.icon}
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true">
                <path d="M7.42 10.05c-.18-.16-.46-.23-.84-.23H6l.02 2.44.04 2.45.56-.02c.41 0 .63-.07.83-.26.24-.24.26-.36.26-2.2 0-1.91-.02-1.96-.29-2.18zM0 4.94v14.12h24V4.94H0zM8.56 15.3c-.44.58-1.06.77-2.53.77H4.71V8.53h1.4c1.67 0 2.16.18 2.6.9.27.43.29.6.32 2.57.05 2.23-.02 2.73-.47 3.3zm5.09-5.47h-2.47v1.77h1.52v1.28l-.72.04-.75.03v1.77l1.22.03 1.2.04v1.28h-1.6c-1.53 0-1.6-.01-1.87-.3l-.3-.28v-3.16c0-3.02.01-3.18.25-3.48.23-.31.25-.31 1.88-.31h1.64v1.3zm4.68 5.45c-.17.43-.64.79-1 .79-.18 0-.45-.15-.67-.39-.32-.32-.45-.63-.82-2.08l-.9-3.39-.45-1.67h.76c.4 0 .75.02.75.05 0 .06 1.16 4.54 1.26 4.83.04.15.32-.7.73-2.3l.66-2.52.74-.04c.4-.02.73 0 .73.04 0 .14-1.67 6.38-1.8 6.68z"></path>
              </svg>
            </a>
            <a
              href="https://www.linkedin.com/in/benhammond404/"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>LinkedIn</span>
              <svg
                className={s.icon}
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"></path>
              </svg>
            </a>
            <a
              href="mailto:BenHammond404@gmail.com"
              target="_blank"
              rel="noopener noreferrer nofollow">
              <span className={s.sr}>E-Mail</span>
              <svg
                className={s.icon}
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true">
                <path d="M12 20.352s-1.096-.108-1.955-.705c-.86-.596-6.58-4.688-6.58-4.688v8.098S3.513 24 4.55 24h14.9c1.036 0 1.085-.942 1.085-.942v-8.1s-5.723 4.092-6.58 4.69c-.86.595-1.955.704-1.955.704zM12 .002S4.925-.23 3.465 7.624v5.35s.06.572 1.67 1.735c1.607 1.162 5.773 4.436 6.867 4.436 1.088 0 5.254-3.273 6.865-4.437 1.607-1.164 1.668-1.737 1.668-1.737v-5.35C19.075-.228 12 .004 12 .004zm4.846 10.536h-9.69V7.624C8.14 3.724 12 3.67 12 3.67s3.863.054 4.846 3.954v2.914z"></path>
              </svg>
            </a>
          </div>
          <span className={s.funny}>
            All left and rights reserved
          </span>
        </div>

        <div className={s.bioContainer}>
          <div className={`h-card ${s.bio}`}>
            <img
              className={`u-photo ${s.image}`}
              src="/webImages/Me.webp"
              alt="The pretty creator"
            />

            <div className={s.textContainer}>
              <Link
                className={`p-name u-url ${s.name}`}
                href="https://bensden.xyz">
                Ben Hammond
              </Link>

              <div className={`p-role ${s.roles}`}>
                <span className={`p-category `}>Developer</span>
                {", "}
                <span className={`p-category `}>Designer</span>
                {", "}
                <span className={`p-category `}>Idiot</span>
              </div>

              <Link
                className={`u-email ${s.email}`}
                href="mailto:BenHammond404@gmail.com">
                BenHammond404@gmail.com
              </Link>

              <div className={s.description}>
                <p className={`p-note`}>A funny code boy</p>
                <span> from</span>
              </div>

              <div className={s.from}>
                <span className={`p-locality ${s.city}`}>
                  Glasgow
                </span>
                {", "}
                <span className={`p-country-name ${s.country}`}>
                  Scotland
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className={s.bottomText}>
        <span>All left and rights reserved</span>
      </div>
    </footer>
  );
};

export default Footer;

//TODO Add H-card stuff for Microformats2
//http://microformats.org/wiki/microformats2
//https://github.com/Janzawada57/PIZDA/blob/079a5d0272c68fa346c2d00390e08243ce40205c/files/en-us/web/html/microformats/index.md

//TODO Follow IndieWeb guide on linking all the stuff to website
//https://indieweb.org/Getting_Started

//TODO Copy this guys integration unless something else is found:
//https://github.com/danillouz/mysite/blob/c6d5300ffcdd7e89ae5cc7c2bdc7d4232ce319e5/src/components/Webmentions/index.tsx
